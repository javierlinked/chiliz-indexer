...
async function main(): Promise<void> {
  ...
  let accumulated: BigNumber = constants.Zero
  contract.on('Transfer', (from: string, to: string, value: BigNumber): void => {
    // native method from BigNumber
    const sum = accumulated.add(value)
    console.log(
      `${from} -> ${to} ${utils.formatUnits(value, 'ether')} ${utils.formatUnits(sum, 'ether')}`
    )
...
async function main2(): Promise<void>  {
  ...
  provider.getTransaction(chztx).then(tx => {
